...
void canBus::can_cycle_recv()
{
    u32 tmp = 0;
     ...
     while(1)
     {
         ...
         {
 #if 1
 //	if((int)Message_cmc.uID == 0x587)
     {
     rcsLog->info("%d--Receive( uID=%x ):\nDATA[0] = %x\tDATA[1] = %x\tDATA[2] = %x\tDATA[3] = %x\nDATA[4] = %x\tDATA[5] = %x\tDATA[6] = %x\tDATA[7] = %x\n",
         Status_cmc,(int)Message_cmc.uID,Message_cmc.arryData[0],Message_cmc.arryData[1],Message_cmc.arryData[2],
         Message_cmc.arryData[3],Message_cmc.arryData[4],Message_cmc.arryData[5],Message_cmc.arryData[6],Message_cmc.arryData[7]);
     }
     //	printf("最大电流阈值:%d\n",(int)Message_cmc.arryData[4]+(int)(Message_cmc.arryData[5]<<8));
 #endif
     AGV_log_speed();
     AGV_log_Current();
     position_to_mileage();
     ...
     {
         Message_cmc.uID -= 0x580;
         canSlaveBoardRxFlag = 1;//节点板发来的消息
         //解析消息
         CMC_slaveBoardMsgPaser();
     }
     if(Message_cmc.uID<=0x058c && Message_cmc.uID>=0x0581)//查询motec电机的速度或者位置
     {
         //举升电机角度
         if((int)Message_cmc.uID== 0x0589&&AGV_position_Get()>=0)
         {
             tmp = (0xFFFFFFFF- AGV_position_Get())/21111;
             if(tmp < 200)
                 {
                     Get_Angle9 = tmp;
                 }
         }
        else if((int)Message_cmc.uID== 0x058A&&AGV_position_Get()>=0)
        {
         tmp=(0xFFFFFFFF- AGV_position_Get())/21111;
         if(tmp < 200)
             {
             ...
             else if((int)Message_cmc.uID== 0x058B&&AGV_position_Get()>=0)
                 {
                     tmp=(0xFFFFFFFF- AGV_position_Get())/21111;
                     if(tmp < 200)
                         {
             ...
             else if((int)Message_cmc.uID== 0x058C&&AGV_position_Get()>=0)
                 {
                     tmp=(0xFFFFFFFF- AGV_position_Get())/21111;
                     if(tmp < 200)
                         {
         ...
         //驱动电机速度
             if((Message_cmc.arryData[1]==0x6c)&&(int)Message_cmc.uID== 0x0581&&AGV_position_Get()>0)
                 {
                     Get_Speed1= ((float)AGV_Speed_Get()/2412.45);
                     FB_speed_flag = 1 ;
                     //printf("1#电机速度：%f\n",Get_Speed1);
                 }
             else if((Message_cmc.arryData[1]==0x6c)&&(int)Message_cmc.uID== 0x0582&&AGV_position_Get()>0)
                 {
                     Get_Speed2=((float)AGV_Speed_Get()/2412.45);
                     FB_speed_flag = 1 ;
                     //printf("2#电机速度：%f\n",Get_Speed2);
                 }
             else if((Message_cmc.arryData[1]==0x6c)&&(int)Message_cmc.uID== 0x0583&&AGV_position_Get()>0)
                 {
                     Get_Speed3=((float)AGV_Speed_Get()/2412.45);
                     FB_speed_flag = 1 ;
                     //printf("3#电机速度：%f\n",Get_Speed3);
                 }
             else if((Message_cmc.arryData[1]==0x6c)&&(int)Message_cmc.uID== 0x0584&&AGV_position_Get()>0)
                 {
                     Get_Speed4=((float)AGV_Speed_Get()/2412.45);
                     FB_speed_flag = 1 ;
                     //printf("4#电机速度：%f\n",Get_Speed4);
                 }
